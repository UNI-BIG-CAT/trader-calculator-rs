							...
							actionList
								...
								.map((action, index) => (
									<tr key={index}>
										...
										<td>{dayjs(action.action_time).format('YYYY-MM-DD HH:mm')}</td>
										<td>
				...
				<div className="dialog-delete-overlay" onClick={closeModal}>
					...
					>
						...
						<div className="delete-btn-container">
							<button className="btn-confirm" onClick={() => handleDeleteAction()}>
								确定
							</button>
							<button className="btn-cancel" onClick={() => closeModal()}>
								取消
			...
			{showActionInfoDialog && (
				<div className="dialog-overlay">
					<div
						className="dialog"
						onKeyDown={(e) => {
							if (e.ctrlKey && e.key === 'Enter' && actionForm.actionInfo.trim()) {
								handleSaveActionInfo();
							}
						}}
						tabIndex={0}
