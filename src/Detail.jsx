...
function Detail({ stockId, stockName, onBack, handleViewActionInfo }) {
	...
	// Toast Hook
	const { toasts, removeToast, showError, showWarning, showSuccess } = useToast();
	/*******************生命周期*********************/
	useEffect(() => {
		if (stockId) {
			getActionList();
	...
	const getTotalExpenditure = (action) => {
		let totalExpenditure = action.current_cost * action.total_position + action.total_fee;
		return totalExpenditure;
	};
	// 计算当前价清仓手续费
	...
	const getNetProfit = (action) => {
		return action.current_price * action.total_position - action.current_cost * action.total_position;
	};
		...
		try {
			...
			if (dialogType == 'add') {
				await invoke('handle_add_position', {
					...
					transactionPosition: parseInt(addOrReduceFormData.transactionPosition),
				});
			} else {
	...
	return (
		<main className="container">
			...
			<div className="detail-container">
				...
				{actionList.length === 0 && <div className="empty-message">暂无操作记录</div>}
			</div>
					...
					>
						...
						<div className="dialog-content">
							...
							<div className="form-group">
								...
								<input
									...
									onChange={handleAddOrReduceInputChange}
									placeholder={`请输入${dialogType === 'add' ? '加仓' : '减仓'}价格`}
									step="0.01"
									max="3001"
								/>
							</div>
						...
						<div className="dialog-actions">
							<button
								...
								disabled={
									!addOrReduceFormData.transactionPosition || !addOrReduceFormData.transactionPrice
								}
			...
			{/* 回退确认框 */}
			{showBackConfirmDialog && (
				<div className="dialog-delete-overlay" onClick={() => setShowBackConfirmDialog(false)}>
					<div
					...
					>
						...
						<div className="delete-btn-container">
							...
							<button className="btn-cancel" onClick={() => setShowBackConfirmDialog(false)}>
								取消
							</button>
						</div>
					</div>
